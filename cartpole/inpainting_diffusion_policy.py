...
a = np.min(dataset, axis=0)[:-1]
b = np.max(dataset, axis=0)[:-1]
...
mask = torch.tensor([[1.0, 1.0, 1.0, 1.0, 0.0]], device=device)
...
for i in range(num_test_episodes):
    ...
    while not done:
        ...
        x = torch.cat((cond,torch.tensor([[1.0]], device=device)),-1).to(torch.float32)
        ...
        if act > 0.5:
            ...
            act = 0
        action = a #* act_scale + act_bias
        obs, reward, done, info = gym_env.step(act)
        num_steps += 1
        if done:
            print("return at the end of the episode : ", episode_return)
            print(num_steps)
